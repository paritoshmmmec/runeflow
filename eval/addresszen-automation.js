...
function parseOptions(argv) {
  const options = {
    ...
    taskQuery: null,
    mode: "both",
    delayMs: 5000, // default 5s gap — Cerebras rate limit buffer
    model: null,
    provider: null,
  };
...
function sleep(ms) {
  ...
  console.error(`[delay] waiting ${ms}ms between runs (Cerebras rate limit buffer)...`);
...
async function runRawBaseline(rawDefinition, runtime, context) {
  ...
  const prompt = [
    `Execute the following Addresszen operation using the provided context: ${context.task_query}`,
    "",
    "Follow the execution patterns inside your docs strictly.",
  ].join("\n");

  const input = { task_query: context.task_query };
