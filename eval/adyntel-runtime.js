...
function buildAdyntelMessages({ prompt, input, docs, context }) {
  const operatorNotes = docs?.trim() ? docs.trim() : "No operator notes were provided.";

  const systemPrompt = [
    "You are an AI tasked with executing Adyntel tasks via Composio.",
...
async function handleAdyntelDraft(payload) {
  const parsed = await dispatchLlm({
    llm: payload.llm,
    ...buildAdyntelMessages(payload),
  });
...
export async function createRuntime(options = {}) {
  const fixture = await loadFixture(options.fixturePath ?? "eval/fixtures/adyntel-automation.default.json");
  ...
  return {
    ...
    llms: {
      cerebras: handleAdyntelDraft,
      anthropic: handleAdyntelDraft,
      openai: handleAdyntelDraft,
    },
  };
}
